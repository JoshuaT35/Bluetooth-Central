...
async def ble_connect_imu(queue):
    rust_path = os.path.join("..", "rust-bluetooth", "target", "release", "rust-bluetooth.exe")
    ...
    # ... rest of the code
        # Read each line printed by the Rust program
    async for line in process.stdout:
        decoded = line.decode().strip()
        try:
            ax, ay, az, time = map(float, decoded.split(','))
            await queue.put((ax, ay, az, time))
        except Exception as e:
            print(f"[Parse error] {decoded} -> {e}")
